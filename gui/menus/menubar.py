...
from ..translate import TranslateFrame
...
class MenuBar(wx.MenuBar):
 def __init__(self, frame):
  ...
  frame.Bind(wx.EVT_MENU, lambda event, frame = frame: TranslateFrame(frame.entry.GetValue()).Show(True), self.file_menu.Append(wx.ID_ANY, '&Translate...\tCTRL+T', 'Translate the current document from print to braille.'))
  ...
  for section in config.config.section_order:
   frame.Bind(wx.EVT_MENU, lambda event, section = section: SimpleConfWxDialog(section).Show(True), self.options_menu.Append(wx.ID_ANY, '&%s...' % section.title, 'View and edit the %s preferences.' % section.title.lower()))
